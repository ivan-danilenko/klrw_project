...
class LeftFreeBimoduleMonoid(CombinatorialFreeModule):
    ...
    One way to describe it is a monoidal object in the category of R-bimodules for some ring R that is free as a left R-module. 
    If R is a (commutative) field, this is an algebra.
    Less formally, it's a ring A that is also an R-bimodule for a given ring R. 
    Compatibility of the structures comes from multiplication being left linear in the first argument, right bilinear in the second argument:
    (ra)*b = r(a*b)
    a*(br) = (a*b)r
    [here r is in R, a,b are in A]
    Also right action on the first argument can be transformed into a left action on the second argument:
    (ar)*b = a*(rb)
    -----
    This happens whenever R is a subring of A [with the assumption that A is free as a left R-module]
    The classical notion of algebra refers to the case when R is *in the center* of A [and, usually, R is a field].
    A classical example is quaternions. Complex numbers are a subring, but quaternions is not an algebra over complex numbers.
    '''
    ...
    def _product_from_product_on_basis_multiply(self, left, right):
        ...
        """
        return self.linear_combination( (self.product_on_basis(mon_mid,mon_right), coeff_left * coeff_mid)
...
class Quaternion(LeftFreeBimoduleMonoid):
    def __init__(self):
        CC = ComplexField()
        super().__init__(CC, ["e", "j"], category=FiniteDimensionalAlgebrasWithBasis(CC))

    def _get_action_(self, other, op, self_on_left):
        #the new action needed only if we act from the right
        if op == operator.mul and self_on_left == True:
