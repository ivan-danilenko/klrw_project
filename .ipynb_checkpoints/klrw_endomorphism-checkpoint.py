...
class LeftKLRWEndomorphismAction(Action):
    def _act_(self, p : FreeAlgebraElement, x : IndexedFreeModuleElement) -> IndexedFreeModuleElement:
        return self.codomain().linear_combination((self._act_by_words_(word,x), coeff)
                                                  for word,coeff in p
                                                 )
    ...
    def _act_by_words_(self, word, x):
        ...
        for braid, poly in x:
            term = self.domain()._from_dict({braid: poly})
            state = braid.left_state()
            for index,n in reversed(sequence):
                for _ in range(n):
                    new_braid = self.actor().simple_endomorphisms_as_klrw_braid(index, state, act_on_left = True)
    ...
    def _act_(self, p : FreeAlgebraElement, x : IndexedFreeModuleElement) -> IndexedFreeModuleElement:
        ...
        for word,coeff in p:
            for braid, poly in x*coeff:
                ...
                for index,n in word:
                    for _ in range(n):
                        new_braid = self.actor().simple_endomorphisms_as_klrw_braid(index, state, act_on_left = False)
...
class KLRWEndomorphismAlgebra(FreeAlgebra_generic):
    ...
    def __init__(self, krlw_algebra, number_of_moving_strands, crossing_prefix = 'y', left_prefix = 'l', right_prefix = 'r'):
        ...
        names_crossing = [crossing_prefix+repr(i) for i in range(1,number_of_moving_strands)]
        names_left = [left_prefix+repr(i) for i in range(1,number_of_moving_strands+1)]
        names_right = [right_prefix+repr(i) for i in range(1,number_of_moving_strands+1)]
        
        super().__init__(self.krlw_algebra.base(), n = 3*number_of_moving_strands - 1, names = names_crossing+names_left+names_right)

        self.simple_endomorphisms = {self(name).leading_item()[0]:(self.crossing_prefix,index+1) 
                                     for index,name in enumerate(names_crossing)
                                    }
        self.simple_endomorphisms |= {self(name).leading_item()[0]:(self.left_prefix,index+1) 
                                      for index,name in enumerate(names_left)
                                     }
        self.simple_endomorphisms |= {self(name).leading_item()[0]:(self.right_prefix,index+1) 
    ...
    def simple_endomorphisms_as_klrw_braid(self, name, state, act_on_left):
        ...
        if prefix == self.crossing_prefix:
            ...
            return self.krlw_algebra.KLRWBraid._element_constructor_(state, word=(i+1,))
            ...
            return self.krlw_algebra.KLRWBraid._element_constructor_(new_state, word=(i+1,))
            ...
            return self.krlw_algebra.KLRWBraid._element_constructor_(new_state, word=(i,))
    ...
    def _repr_(self):
       ...
       """
       return "Endomorphism algebra of a KLRW algebra with {} moving strands.".format(self.number_of_moving_strands)
