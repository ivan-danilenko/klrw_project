...
class KLRWAlgebraWithTests(KLRWAlgebra):
    def check(self):
        ...
        self.check_associativity(self.basis_over_dots_and_center(),self.gens_over_dots(),self.basis_in_dots_modulo_center())
        #self.check_associativity(self.basis_over_dots_and_center(),self.basis_over_center(),self.basis_in_dots_modulo_center())
        ...
        for a,b in product(first_family, second_family):
            ...
            if check_right_state:
                for braid_ab,__ in ab:
                    for braid_b,_ in b: 
                        assert braid_ab.right_state() == braid_b.right_state(), "Right state don't match for {} braid in {} = {}*{}".format(braid_ab, ab, a,b)
            if check_left_state:
                for braid_ab,__ in ab:
                    for braid_a,_ in a:
                        assert braid_ab.left_state() == braid_a.left_state(), "Left state don't match for {} in {} = {}*{}".format(braid_ab, ab, a,b)
            if check_braids:
                ...
                for braid_ab,coeff_ab in ab:
                    for braid_a,coeff_a in a: 
                        for braid_b,coeff_b in b:
                            term_ab = self._from_dict({braid_ab:coeff_ab})
                            term_a = self._from_dict({braid_a:coeff_a})
                            term_b = self._from_dict({braid_b:coeff_b})
                            ...
                            assert degree_ab == degree_a + degree_b, "Degrees don't match for {} in {} = {}*{}".format(term_ab, ab, a,b)
            if count%1000000 == 0:
                print("So far checked ", count, " relations.")
...
#    def check_commutativity_with_center(self):
#        commutes = True
#        relations = 0
#        iterator_over_elements_in_center = chain([self.base().one()],self.center_gens())
#        for x,c in product(self.basis_over_center(),iterator_over_elements_in_center):
#            if x*c != c*x:
#                commutes = False
#                print("Issue with")
#                print(x,"*",c)
...
#            relations += 1
#
#        print("Commutes with the center:", commutes)
#        print("Relations checked:", relations)
    ...
    def check_linearity_of_right_dots_action_over_center(self):
        ...
        '''
        count = 0
        iterator_over_elements_in_center = chain([self.base().one(),-self.base().one(),2*self.base().one()],self.center_gens())
        ...
        for x,c,d in product(self.basis_over_center(),iterator_over_elements_in_center,self.basis_in_dots_modulo_center()):
            assert x*(c*d) == (c*x)*d, "Issue with {0}*({1}*{2}) == ({1}*{0})*{2}".format(x,c,d)
            count += 1
            if count%1000000 == 0:
                print("So far checked ", count, " relations.")
    ...
    def check_relations(self):
        ...
        Relation xx means relation in formula (2.xx) in https://arxiv.org/pdf/1111.1431.pdf
            ...
            for i in range(len(state)-2):
                if state[i] == state[i+1] and state[i+1] == state[i+2]:
                    if not state[i].is_framing():
                        ...
                        assert x*y*x == y*x*y, print("Relation 8b does not hold: {0}*{1}*{0} != {1}*{0}*{1}".format(x,y))
            ...
            for i in range(len(state)-1):
                if state[i] == state[i+1]:
                    if not state[i].is_framing():
                        ...
                        dot_right = self.base().variables[state[i],dot_index+1].monomial
                        ...
                        assert dot_left*x - x*dot_right == r*self.idempotent(state), "{0}*{1}-{1}*{2} != {3}".format(dot_left,x,dot_right, r*self.idempotent(state))
            ...
            for i in range(len(state)-1):
                if state[i] == state[i+1]:
                    if not state[i].is_framing():
                        ...
                        r = self.base().variables[state[i]].monomial
                        x = self.KLRWmonomial(state = state, word = (i+1,))
                        assert x*dot_left - dot_right*x == r*self.idempotent(state)
                ...
                if state[i] != state[i+1]:
                    if not state[i].is_framing() or not state[i+1].is_framing():
                        ...
                        if d_ij > 0:
                            ...
                            t_ji = self.base().variables[state[i+1],state[i]].monomial
                            dot_left_index = state.index_among_same_color(i)
                            dot_right_index = state.index_among_same_color(i+1)
                            dot_left = self.base().variables[state[i],dot_left_index].monomial
                            dot_right = self.base().variables[state[i+1],dot_right_index].monomial
                            coeff = (t_ij*(dot_left**d_ij) + t_ji*(dot_right**d_ji))
                        else:
                            coeff = t_ij
                        x1 = self.KLRWmonomial(state = state.act_by_s(i+1), word = (i+1,))
                        x2 = self.KLRWmonomial(state = state, word = (i+1,))
                        assert x1*x2 == coeff*self.idempotent(state), print("Relation 9 does not hold: {0}*{1} == {2} != {3}".format(x1,x2,x1*x2,coeff*self.idempotent(state)))
        ...
        for state in self.KLRWBraid.KLRWstate_set:
            for i in range(len(state)-2):
                if state[i] != state[i+2] or -self.quiver[state[i],state[i+1]]<=0:
                    number_of_framings = sum(1 for color in state[i:i+3] if color.is_framing())
                    if number_of_framings<=1:
                        #print(state, i)
                        x3 = self.KLRWmonomial(state = state, word = (i+1,))
                        x2 = self.KLRWmonomial(state = state.act_by_s(i+1), word = (i+2,))
                        x1 = self.KLRWmonomial(state = state.act_by_s(i+1).act_by_s(i+2), word = (i+1,))
                        y3 = self.KLRWmonomial(state = state, word = (i+2,))
                        y2 = self.KLRWmonomial(state = state.act_by_s(i+2), word = (i+1,))
                        y1 = self.KLRWmonomial(state = state.act_by_s(i+2).act_by_s(i+1), word = (i+2,))
                        assert x1*x2*x3 == y1*y2*y3, print("Relation 13 does not hold: {0}*{1}*{2} = {3} != {4} = {5}*{6}*{7}".format(x1,x2,x3,x1*x2*x3,y1*y2*y3,y1,y2,y3))
        ...
        for state in self.KLRWBraid.KLRWstate_set:
            for i in range(len(state)-2):
                if state[i] == state[i+2] and -self.quiver[state[i],state[i+1]]>0:
                    if not state[i].is_framing():
                        x3 = self.KLRWmonomial(state = state, word = (i+1,))
                        x2 = self.KLRWmonomial(state = state.act_by_s(i+1), word = (i+2,))
                        x1 = self.KLRWmonomial(state = state.act_by_s(i+1).act_by_s(i+2), word = (i+1,))
                        ...
                        dot_left_index = state.index_among_same_color(i)
                        dot_left = self.base().variables[state[i],dot_left_index].monomial
                        dot_right = self.base().variables[state[i],dot_left_index+1].monomial
                        coeff = t_ij*r_i*sum(dot_left**l * dot_right**(d_ij-l-1) for l in range(d_ij))
                        #print(r_i,d_ij,t_ij, dot_left,dot_right)
                        #print([(l,d_ij-l-1, dot_left**l * dot_right**(d_ij-l-1)) for l in range(d_ij)])
                        #print(coeff)
                        assert x1*x2*x3 - y1*y2*y3 == coeff*self.idempotent(state), print("Relation 14 does not hold: {0} - {1} != {2}".format(x1*x2*x3,y1*y2*y3,coeff*self.idempotent(state)))
                        assert x1*x2*x3 != self.zero()
                        cases14+=1
    ...
    def check_associativity(self, first_family, second_family, third_family):
        ...
        If max_word_length is given, for each family only elements that have all terms of length less than this number are considered
        """
            
        associativity_holds = True
        count = 0
        for a,b,c in product(first_family,second_family,third_family):
            assert a*(b*c) == (a*b)*c, "Issue with {}, {}, {}\n".format(a,b,c) + "bc = {}\n".format(b*c) + "ab= {}\n".format(a*b)
            count += 1
            if count%1000000 == 0:
                print("So far checked ", count, " relations.")
