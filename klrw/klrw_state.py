...
class KLRWstate(ElementWrapper):
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            # we modify the state, but don't want to recompute the dimensions.
            # We make a set without dimension checks.
...
class KLRWstate_set(UniqueRepresentation, Set_generic):
    ...
    def check_element(self, element):
        ...
        for node, dim in self._dimensions.items():
            assert dim == element.count(
                node
            ), "The node dimension for {} is incorrect.".format(node)
    ...
    def act_by_s(self, i, element):
        ...
        # we have a shift because the elements in a tuple
        # are numbered by 1, ..., number_of_strands
